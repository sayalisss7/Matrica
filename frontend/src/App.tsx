...
const HomePage = ({ isDarkMode }: ThemeProps) => {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL || 'https://matrica-backend.jollyplant-fd7887aa.centralindia.azurecontainerapps.io'}/api/dashboard/recommendations`)
      .then(res => {
...
const Sponsors = ({ isDarkMode }: ThemeProps) => {
  ...
  const runMatch = async () => {
    ...
    setError('')
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL || 'https://matrica-backend.jollyplant-fd7887aa.centralindia.azurecontainerapps.io'}/api/match_sponsor`, {
...
const Chat = ({ isDarkMode, messages, setMessages }: ChatProps) => {
  ...
  const handleSend = async (overrideInput?: string, skipPush?: boolean) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL || 'https://matrica-backend.jollyplant-fd7887aa.centralindia.azurecontainerapps.io'}/api/chat`, { query: textToSend }, {
