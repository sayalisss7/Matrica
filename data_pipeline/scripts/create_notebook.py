...
print("Gold layer processed and saved!")"""
    ...
    # PostgreSQL Load
    nb.cells.append(nbf.v4.new_markdown_cell("## 4. Final Destination (PostgreSQL)\nLoad the Gold Parquet tables directly into the Neon PostgreSQL database for our backend FastAPI application."))
    code_postgres = """# Helper function to read parquet with pandas and save to PostgreSQL
...
print("Database load complete! The AI Agent is ready to query PostgreSQL.")"""
    nb.cells.append(nbf.v4.new_code_cell(code_postgres))
    ...
    # Save Notebook
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    notebook_path = os.path.join(BASE_DIR, 'notebooks', 'medallion_pipeline.ipynb')
